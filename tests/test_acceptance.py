...
def flagged_ms(request, tmp_path_factory):
  ...
  args = ["tricolour", "-fs", "total_power", "-c", os.path.join(test_directory, "custom.yaml"), ms_filename]
...
def test_flag_count(flagged_ms, tol):
  ...
  print("Percent flagged for 3C286: %.3f%%" % (100.0 * flagged_ratio))
  ...
  print("Percent flagged for PKS1934-63: %.3f%%" % (100.0 * flagged_ratio))
  assert flagged_ratio < tol
...
def test_bandwidth_flagged(flagged_ms, tol):
  ...
  print("Percent bandwidth flagged for 3C286: %.3f%%" % (100.0 * flagged_ratio))
  assert flagged_ratio < tol
  ...
  print("Percent bandwidth flagged for PKS1934-63: %.3f%%" % (100.0 * flagged_ratio))
  assert flagged_ratio < tol
