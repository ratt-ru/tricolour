...
def load_mask(filename, dilate):
  ...
  if not (np.issubdtype(mask.dtype[0], bool) and np.issubdtype(mask.dtype[1], np.float64)):
    raise ValueError(
      f"Mask {filename} is not a valid static mask with labelled channel axis [dtype == (bool, float64)]"
    )
  ...
  log.info(
    "Loaded mask {0:s} {1:s} with {2:.2f}% flagged bandwidth between {3:.3f} and {4:.3f} GHz".format(
      filename,
    ...
    for path in paths:
      ...
      if os.path.exists(path):
        if os.path.isdir(path):
          file_paths.extend(
            sorted([os.path.join(path, p) for p in os.listdir(path) if os.path.splitext(p)[1].lower() in file_exts])
          )
        else:
