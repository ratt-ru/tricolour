...
def window_stats(flag_window, ubls, chan_freqs, antenna_names, scan_no, field_name, ddid, nchanbins):
  ...
  for ch_i, ch in enumerate(bins_edges[:-1]):
    sel = np.logical_and(chan_freqs >= bins_edges[ch_i], chan_freqs < bins_edges[ch_i + 1])
...
def combine_baseline_window_stats(baseline_stat_list, prev_stats):
  """
...
def combine_window_stats(window_stats):
  ...
  final_stats : :class:`dask.Array`
      Dask array containing a single :class:`WindowStatistics` object, or
      ``None`` if ``window_stats`` is empty (e.g. a worker processed no
      partitions).
  """
  if not window_stats:
    return None

  result = window_stats[0].copy()

  for arg in window_stats[1:]:
    result.update(arg)
...
class WindowStatistics(object):
  ...
  def summarise_stats(final, original):
    ...
    for a in final._counts_per_ant:
      l.append(
        "\t {0:s}: {1:.3f}%, original {2:.3f}%".format(
          a,
          final._counts_per_ant[a] * 100.0 / final._size_per_ant[a],
    ...
    for s in final._counts_per_scan:
      l.append(
        "\t {0:s}: {1:.3f}%, original {2:.3f}%".format(
          s,
          final._counts_per_scan[s] * 100.0 / final._size_per_scan[s],
    ...
    for f in final._counts_per_field:
      l.append(
        "\t {0:s}: {1:.3f}%, original {2:.3f}%".format(
          f,
          final._counts_per_field[f] * 100.0 / final._size_per_field[f],
    ...
    for b in final._counts_per_bl:
      l.append(
        "\t {0:s}: {1:.3f}%, original {2:.3f}%".format(
          b,
          final._counts_per_bl[b] * 100.0 / final._size_per_bl[b],
          original._counts_per_bl[b] * 100.0 / original._size_per_bl[b],
        )
      )
    ...
    for d in final._counts_per_ddid:
      ...
      l.append("\t {0:s}: {1:s}%".format(d, ratio_str))
