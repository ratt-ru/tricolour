...
def _asbool_impl_jit(data):
  ...
  if isinstance(data, np.ndarray):
    ...
    if data.dtype.itemsize == 1:
      ...
      return data.astype(np.bool_)
  elif isinstance(data.dtype, numba.types.Boolean) or (
...
def _sum_threshold1d(input_data, input_flags, output_flags, windows, outlier_nsigma, rho, chunks):
  ...
  for ci in range(chunks.size - 1):
    ...
    padded_slice = slice(
      max(chunks[ci] - np.max(windows) + 1, 0), min(chunks[ci + 1] + np.max(windows) - 1, input_data.size)
    )
    ...
    rel_slice = slice(chunk_slice.start - padded_slice.start, chunk_slice.stop - padded_slice.start)
...
def _sum_threshold(input_data, input_flags, axis, windows, outlier_nsigma, rho, chunks=None):
  ...
  if axis < 0 or axis >= input_data.ndim:
    ...
    for i in range(input_data.shape[0]):
      _sum_threshold1d(input_data[i], input_flags[i], output_flags[i], windows, outlier_nsigma, rho, chunks)
  elif axis == 0:
    ...
    for i in range(0, input_data.shape[1], step):
      ...
      _sum_threshold1d(
        input_data[:, sub], input_flags[:, sub], output_flags[:, sub], windows, outlier_nsigma, rho, chunks
      )
  else:
...
def _get_flags_impl(
  ...
  for cp in range(n_cp):
    for t in range(n_time):
      for f in range(n_freq):
        out_flags[cp, t, f] = tmp_flags[cp, t, f] or np.isnan(in_data[cp, t, f])
...
def _get_baseline_flags(
  ...
  spec_flags = _sum_threshold(spec_data, spec_flags, 1, windows_freq, outlier_nsigma, rho, freq_chunk_ends)
  ...
  _unaverage_freq(flags, freq_extend, average_freq, flag_all_time_frac, flag_all_freq_frac, out_flags)
...
def uvcontsub_flagger(vis, flags, major_cycles=5, or_original_from_cycle=1, taylor_degrees=20, sigma=5):
  ...
  for mi in range(major_cycles):
    ...
    for cp in range(vis.shape[0]):
      ...
      diff = np.abs(np.abs(flagged_absresidual) - np.nanmedian(np.abs(flagged_absresidual)))
...
def sum_threshold_flagger(
  ...
  windows_time = np.array([w for w in windows_time if w <= ntime], np.int_)
  ...
  for i in range(num_major_iterations):
    ...
    iter_flags = np.logical_or(iter_flags, out_flags)
...
class SumThresholdFlagger(object):
  ...
  def _get_flags(self, in_data, in_flags, out_flags):
    ...
    averaged_channels = (int(nchan) + int(self.average_freq) - 1) // int(self.average_freq)

    # Set up frequency chunks
    freq_chunk_ends = np.linspace(0, averaged_channels, self.freq_chunks + 1).astype(np.int_)

    # Clip the windows to the available time and frequency range
    windows_time = np.array([w for w in self.windows_time if w <= ntime], np.int_)
